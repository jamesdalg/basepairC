#' Mean Pooling with Parallel Processing Function
#'
#' This function performs mean pooling on a matrix with parallel processing support.
#'
...
#'
#' @return A pooled numeric matrix using mean pooling.
#'
#' @examples
#' mat <- matrix(1:16, nrow = 4)
#' mean_pooling_parallel(mat, factor = 2)
#'
#' @export
mean_pooling_parallel <- function(mat, factor,ncores=parallel::detectCores()) {
  ...
  pooling_function <- function(idx) {
    i <- as.numeric(idx[1])
    j <- as.numeric(idx[2])
    return(mean(mat[i:(i+factor-1), j:(j+factor-1)]))
  }
