...
#' @export
mnase_bias_win <- function(chromosome, position, genome = BSgenome.Mmusculus.UCSC.mm10::BSgenome.Mmusculus.UCSC.mm10, window_size = 6, n_cores = parallel::detectCores() / 2,mnase_model=NULL) {
  ...
  data.table::data.table(chr = unique(as.character(chromosome)), start = seq(min(position)-window_size, max(position)+window_size)) %>%
    dplyr::mutate(seq = gene_locus_seq %>% as.character() %>% strsplit("") %>% unlist(), end = start) %>%
    dplyr::select(chr, start, end, seq) -> gene_locus_dt_bp
   #browser()
  pbmcapply::pbmclapply((window_size+1):(nrow(gene_locus_dt_bp)-window_size), function(i) {
    # print(i)
    ...
    gene_locus_dt_bp[min_window_pos:max_window_pos, "seq"] %>% unlist() %>%  paste(collapse="")-> window_seq
