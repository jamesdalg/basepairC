...
pbapply::pblapply(rev(1:nrow(dir_df)),function(d){
  tryCatch({
...
tryCatch({
    starting_values <- basepairC.core::bootstrap_gam_parallel_speedglm(
      data = ref_data_df,
      formula = count ~ -1 + model.matrix( ~ sham_trt_factor+sample,contrasts.arg = list(sample=contr.sum,sham_trt_factor=contr.treatment)), 
      family = quasipoisson(),
      fraction = 0.01, 
      n_bootstraps = 5
    ) %>% .$avg_coefs 
...
speedglm::speedglm(count ~ -1 + model.matrix( ~ sham_trt_factor+sample,contrasts.arg = list(sample=contr.sum,sham_trt_factor=contr.treatment)),trace=T, data=ref_data_df,family=quasipoisson(),start=starting_values)->qp_model_sham_ref
starting_values=NULL
tryCatch({
    starting_values <- basepairC.core::bootstrap_gam_parallel_speedglm(
      data = alt_data_df,
...
speedglm::speedglm(count ~ -1 + model.matrix( ~ sham_trt_factor+sample,contrasts.arg = list(sample=contr.sum,sham_trt_factor=contr.treatment)),trace=T, data=alt_data_df,family=quasipoisson(),start=starting_values)->qp_model_sham_alt
...
tryCatch({
    starting_values <- basepairC.core::bootstrap_gam_parallel_speedglm(
      ...
      formula = count ~ -1 + model.matrix( ~ alt_factor+sample,contrasts.arg = list(sample=contr.sum,alt_factor=contr.treatment)), 
      family = quasipoisson(),
      fraction = 0.01, 
      n_bootstraps = 5
    ) %>% .$avg_coefs 
...
speedglm::speedglm(count ~ -1 + model.matrix( ~ alt_factor+sample,contrasts.arg = list(sample=contr.sum,alt_factor=contr.treatment)),trace=T, data=full_data_df_true,family=quasipoisson(),start=starting_values)->qp_model_true_speedglm
...
data.table::fwrite(x = model_df_complete_glmmtmb_ziqp,paste0("/code/R/allele_spec_comparisons/results/intermediate_all_jan_16/allele_spec_comparison_neg_control_speedglm_qp_results_glm_sample_spec_intercept_with_sham_",dir_df$dataset[d],".tsv"),sep="\t")
model_df_complete_glmmtmb_ziqp %>% dplyr::select(-est_abs_mean,-est_abs_sd,-est_mean,-est_sd,-mean_se,-mean_abs_stat,-mean_p,-logp_mean_stat) %>% dplyr::filter(analysis=="true") %>% data.table::fwrite(paste0("/code/R/allele_spec_comparisons/results/intermediate_all_jan_16/allele_spec_comparison_neg_control_speedglm_qp_results_glm_sample_spec_intercept_true_analysis_",dir_df$dataset[d],".tsv"),sep="\t")
model_df_complete_glmmtmb_ziqp %>% dplyr::select(-estimate,-std.error,-statistic,-p.value) %>% dplyr::filter(analysis=="sham") %>% data.table::fwrite(paste0("/code/R/allele_spec_comparisons/results/intermediate_all_jan_16/allele_spec_comparison_neg_control_speedglm_qp_results_glm_sample_spec_intercept_sham_analysis_",dir_df$dataset[d],".tsv"),sep="\t")

model_df_complete_glmmtmb_ziqp %>% dplyr::select(-estimate,-std.error,-statistic,-p.value) %>% dplyr::filter(analysis=="sham")  %>% tidyr::separate(sep="\\)\\)",into=c("matrix","term"),col=term) %>% dplyr::filter(term=="sham_trt_factorsham_trt") %>% dplyr::mutate(analysis=as.character(analysis)) %>% dplyr::select(term,est_abs_mean,est_mean,est_abs_sd,rsid) %>% dplyr::inner_join(
  model_df_complete_glmmtmb_ziqp %>% dplyr::select(-est_abs_mean,-est_abs_sd,-est_mean,-est_sd,-mean_se,-mean_abs_stat,-mean_p,-logp_mean_stat) %>% dplyr::filter(analysis=="true") %>% tidyr::separate(sep="\\)\\)",into=c("matrix","term"),col=term)   %>% dplyr::filter(term=="alt_factoralt") %>% dplyr::mutate(analysis=as.character(analysis)) %>% dplyr::select(rsid,term,estimate,std.error,p.value,nonzero_percent_per_col,nonzero_counts_per_col),
  by="rsid"
) -> sham_trt_merged_ssi
data.table::fwrite(sham_trt_merged_ssi,paste0("/code/R/allele_spec_comparisons/results/merged_all_jan_16_2026/allele_spec_comparison_neg_control_speedglm_qp_results_glm_sample_spec_intercept_sham_trt_vs_true_alt_effect_",dir_df$dataset[d],"_ssi.tsv"),sep="\t")
return(sham_trt_merged_ssi)
  },error=function(e){
    basepairC.core::message_parallel(paste0("Error with dataset ",dir_df$dataset[d],": ",e))

    writeLines(text = paste0(e),con=paste0("/code/R/allele_spec_comparisons/dataset_errors_jan_16/",dir_df$dataset[d],".error"))
    return(data.frame())
  })
}) %>% data.table::rbindlist(fill=TRUE)->all_datasets_sham_trt_vs_true_alt_effect
data.table::fwrite(all_datasets_sham_trt_vs_true_alt_effect,"/code/R/allele_spec_comparisons/results/merged_all_jan_16_2026/allele_spec_comparison_neg_control_speedglm_qp_results_glm_sample_spec_intercept_sham_trt_vs_true_alt_effect_ALL_datasets_ssi.tsv",sep="\t")
